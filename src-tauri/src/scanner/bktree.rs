/// Arena-based BK-tree for u64 values with Hamming distance metric.
/// Reduces perceptual-hash duplicate search from O(n²) to O(n log n) average.
pub struct BkTree {
    // (hash, original_index, children[(hamming_dist_to_parent, node_index)])
...
impl BkTree {
    ...
    pub fn find_within(&self, query: u64, threshold: u32) -> Vec<usize> {
        ...
        while let Some(node_idx) = stack.pop() {
            ...
            // Triangle-inequality pruning: skip branches where no candidate can be within threshold.
